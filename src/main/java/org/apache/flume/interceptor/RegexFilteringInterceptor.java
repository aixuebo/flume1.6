...
public class RegexFilteringInterceptor implements Interceptor {
  ...
  public Event intercept(Event event) {
    ...
    if (!excludeEvents) {//excludeEvents为false,表示要正则表达式匹配的body内容的事件
      if (regex.matcher(new String(event.getBody())).find()) {//只要body匹配上正则表达式.该事件就通过,否则返回null
        return event;
      }
      else {
        return null;
      ...
      if (regex.matcher(new String(event.getBody())).find()) {//只要body不通过该正则表达式,该事件就通过.如果匹配了正则表达式,则返回null
        return null;
      }
      else {
        return event;
