 ...
 * 在header中设置timestampkey,如果header中存在,并且preserveExisting为true,则不需要重新设置,而仅仅需要保存header中现存的就可以
 */
public class TimestampInterceptor implements Interceptor {

  private final boolean preserveExisting;//preserveExisting为true表示需要保持header中的,不需要重新设置
  ...
  public Event intercept(Event event) {
    ...
    if (preserveExisting && headers.containsKey(TIMESTAMP)) {//preserveExisting表示需要保持header中的,不需要重新设置
