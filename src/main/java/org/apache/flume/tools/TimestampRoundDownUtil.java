...
public class TimestampRoundDownUtil {
  ...
  private static Calendar roundDownField(
    ...
    int fieldVal = cal.get(field);//当前field单位对应的时间
    System.out.println(fieldVal);
    int remainder =  (fieldVal % roundDown);//比如fieldVal=30分,而roundDown=25分,最终结果5
    System.out.println(remainder);
    cal.set(field, fieldVal - remainder);//fieldVal - remainder = 30 - 5 = 25,即每隔25分钟进行划分一次
    return cal;
  }
