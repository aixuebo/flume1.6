...
public class SequenceFileSerializerFactory {
  ...
  /**
   * 根据formatType找到对应的序列化类
   * @param formatType,可以是HDFSWritableSerializer.Builder、HDFSTextSerializer.Builder、或者自定义实现了SequenceFileSerializer接口的方法,该接口定义了key-value类型,以及传入一个事件,进行存储的函数
   * @param context
   * @return
   */
