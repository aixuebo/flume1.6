...
public class PersistableEvent implements Event {
  ...
  private byte[] basePayload;//存储body的数据
  private byte[] spillPayload;//存储body超出限制后的字节数据
  private List<HeaderEntry> headers;//存储所有的header信息
  ...
  protected static class SpillableString {
    ...
    private String base;//一定限制内的字符串
    private String spill;//超出一定限制字符后的部分字符换
