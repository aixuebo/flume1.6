...
public abstract class TransactionEventRecord implements Writable {
  ...
  private final long transactionID;//事物ID
  private long logWriteOrderID;//同一个事物ID下的每一个步骤ID
  ...
  static {
    ...
    try {
      builder.put(Type.PUT.get(),
          Put.class.getDeclaredConstructor(Long.class, Long.class));//需要事物ID和logWriteOrderID外,还需要FlumeEvent对象,保存待存储的header集合和body内容
      builder.put(Type.TAKE.get(),
          Take.class.getDeclaredConstructor(Long.class, Long.class));//需要事物ID和logWriteOrderID外,还需要FlumeEvent对象存储在文件ID和该文件的偏移量
      builder.put(Type.ROLLBACK.get(),
          Rollback.class.getDeclaredConstructor(Long.class, Long.class));//仅仅需要事物ID和logWriteOrderID即可
      builder.put(Type.COMMIT.get(),
          Commit.class.getDeclaredConstructor(Long.class, Long.class));//需要事物ID和logWriteOrderID,还需要知道提交的任务类型,Take|Put
    } catch (Exception e) {
