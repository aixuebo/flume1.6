...
final class FlumeEventQueue {
   ...
   * safe methods only.
   * 标示还没有被提交的存储到该队列中,有两个实例,一个是take,一个是put
   */
  class InflightEventWrapper {
	  //存储事务ID,以及该事务对应的事件的文件ID以及在文件中的偏移量
    ...
    private volatile boolean syncRequired = false;//false表示同步完成,没有改动
    //存储事务ID,以及该事务对应的事件的文件ID
     ...
     * @param transactionID
     * @param pointer 标示该事件对应的文件ID和该文件的偏移量
     */
    ...
    public void serializeAndWrite() throws Exception {
      ...
      try {
    	  //计算需要的存储空间 inflightEvents.keySet().size() 表示需要多少个事务,因为每一个事务还需要存储有多少个事件,因此需要*2,然后事件的大小,最终*8表示需要的字节数,最后+16表示校验和的字节数
        ...
        file.setLength(expectedFileSize);//设置文件大小
        ...
        file.seek(0);
        final ByteBuffer buffer = ByteBuffer.allocate(expectedFileSize);//分配输出流空间
        LongBuffer longBuffer = buffer.asLongBuffer();//创建输出流
        for (Long txnID : inflightEvents.keySet()) {//循环每一个事务ID
          Set<Long> pointers = inflightEvents.get(txnID);//获取该事务ID对应的事件集合
          longBuffer.put(txnID);//存储事务ID
          longBuffer.put((long) pointers.size());//存储属于该事务的事件ID个数
          ...
          long[] written = ArrayUtils.toPrimitive(//将每一个事件对应的文件ID和偏移量组成的long写入输出流
